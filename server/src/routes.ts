    ...
    try {
        ...
        const response = await requestGpt({
            prompt: "차량 이름=" + encar.carName +
                    ...
                    "차량 연식대비 주행거리가 연평균 7500 km 미만 이면서 연식이 10년 이상이라면, 낮은 점수를 줘." +
                    ...
                    "연식 대비 운전자(사용자) 교체가 잦다면 낮은 점수를 줘" +
                    "차량이 렌트 이력이 있다면 낮은 점수를 줘" +
                    "리콜대상의 차량인데, 리콜을 하지 않았다면 낮은 점수를 줘" +
                    "엔카가 진단하지 않은 차량은 낮은 점수를 줘" +
                    "골격에 대한 교체 이력이 있거나 렌트이력이 있다면, 구입하기엔 가장 리스크가 큰 차량이므로 해당 차량을 추천하지 말아줘." +
                    "기준을 직접적으로 언급하지 말고 차량을 추천할지 말지만 알려주고, 그 이유를 100자 내외로 요약해서 말해줘. 존댓말로 답변해줘."
            });
